...
type ThirdPartyOptions struct {
	...
	ReceiptId             string      `json:"receipt_id,omitempty"`             // 华为回执 ID 仅华为通道有效。输入一个唯一的回执 ID 指定本次下行消息的回执地址及配置，该回执 ID 可以在 华为回执参数配置 中查看。
	CallbackId            string      `json:"callback_id,omitempty"`            // vivo回执 ID 仅vivo通道有效。输入一个唯一的回执 ID 指定本次下行消息的回执地址及配置，该回执 ID 可以在 vivo回执参数配置 中查看。
	TargetUserType        int         `json:"target_user_type,omitempty"`       // 华为消息类型 仅华为通道有效。 0：普通消息（默认值） 1：测试消息。每个应用每日可发送该测试消息 500 条且不受 每日单设备推送数量上限要求 。
}
